...
#[tauri::command]
pub fn annihilate_book(
    existing_json: &str,
    file_path: &str,
    book_data: Book,
) -> AnnihilateBookResult<()> {
    ...
    let book_being_deleted = b
        .books
        .iter()
        .position(|b| b.uid == book_data.uid)
        .expect("Error finding book to delete");
    ...
    println!(
        "successfully removed {} from {} written at {}",
        book_being_deleted.title, j, file_path
    );
