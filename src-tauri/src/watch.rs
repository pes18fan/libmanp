...
pub fn watch(window: Window, file_path: String) {
    ...
    if watcher_lock.is_none() {
            ...
            match res {
                Ok(events) => {
                    events.iter().for_each(|e| {
                        ...
                        window.emit("file-change", Placeholder { msg: "placeholder".into() }).unwrap();
                    });
                },
                Err(errs) => {
        ...
        thread::spawn(move || {
            loop {
                debouncer.watcher().watch(Path::new(&file_path), RecursiveMode::Recursive).expect("Error while watching file");
            }
        });
